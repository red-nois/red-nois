...
def start():
	...
	clean_train_filenames, clean_val_filenames = mozilla_dataset.get_train_val_filenames()

	us8k_dataset = UrbanSound8K(urbansound_root, val_dataset_size=200)
	noisy_train_filenames, noisy_val_filenames = us8k_dataset.get_train_val_filenames()
	...
	val_dataset = Dataset(clean_val_filenames,
						  noisy_val_filenames, **settings)
	val_dataset.create_tf_records(prefix='val', subset_size=2000)

	train_dataset = Dataset(clean_train_filenames,
							noisy_train_filenames, **settings)
	...
	test_dataset = Dataset(clean_test_filenames,
						   noisy_test_filenames, **settings)
