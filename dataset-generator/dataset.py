...
class Dataset:
	...
	def _scale_phase_difference(self, clean_spectral_magnitude, clean_phase, noisy_phase):
	...
	def _randomly_crop_audio(self, audio, duration):
		audio_duration_seconds = librosa.core.get_duration(
			audio, self.sample_rate)

		if duration >= audio_duration_seconds:
			return audio

		audio_duration_ms = math.floor(
			audio_duration_seconds * self.sample_rate)
	...
	def process_audio_in_parallel(self, clean_file_path):
		...
		noisy_input = self._add_noise_to_clean_audio(
			clean_audio, noisy_audio)

		noisy_input_features = FeatureExtractor(noisy_input, window_length=self.window_length, overlap=self.overlap,
		...
		clean_input_features = FeatureExtractor(clean_audio, window_length=self.window_length, overlap=self.overlap,
												 sample_rate=self.sample_rate)
	...
	def create_tf_records(self, *, prefix, subset_size, parallel=True):
		...
		for i in range(0, len(self.clean_file_paths), subset_size):
			...
			tfrecord_filename = prefix + \
				'_' + str(count) + '.tfrecords'
			...
			if parallel:
				outputs = pool.map(self.process_audio_in_parallel,
								   clean_file_paths_subset)
			else:
				outputs = [self.process_audio_in_parallel(
					file_path) for file_path in clean_file_paths_subset]
			...
			for output in outputs:
				...
				for x_, y_, p_ in zip(noisy_stft_magnitude_features, clean_stft_magnitude, noisy_stft_phase):
