...
class UrbanSound8K:
	...
	def _get_urban_sound_8k_files(self):
		urbansound_metadata = pd.read_csv(os.path.join(
			self.base_path, 'metadata', 'UrbanSound8K.csv'))
	...
	def get_filenames_with_class_id(self, metadata):
		...
		for c in self.class_ids:
			...
			class_files = [os.path.join(self.base_path, 'audio', 'fold' + str(file[1]), file[0]) for file in
						   class_files]
			print("There are", len(
				class_files), "files in class", str(c))
	...
	def get_train_val_filenames(self):
		...
		urbansound_train_filenames = self.get_filenames_with_class_id(
			urbansound_train)
		...
		urbansound_train = urbansound_train_filenames[:-
	...
	def get_test_filenames(self):
		...
		urbansound_test_filenames = self.get_filenames_with_class_id(
			urbansound_test)
